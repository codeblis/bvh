...
import { NEWS, getNoticia, getNoticiasRelacionadas, NEWS_CONTENT } from "@/data/noticias";
...
export default async function NoticiaPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const article = getNoticia(slug);
  if (!article) notFound();

  const content = NEWS_CONTENT[slug] ?? [article.excerpt];
  const relacionadas = getNoticiasRelacionadas(article);
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="border-b border-border" style={{ background: "var(--gradient-hero)" }}>
        <div className="mx-auto max-w-3xl px-6 py-8 md:py-12">
          <Link href="/noticias" className="inline-flex items-center gap-1.5 text-[11px] uppercase tracking-widest text-muted-foreground hover:text-primary transition">
      ...
      <article className="mx-auto max-w-3xl px-6 py-8 md:py-12">
        ...
        <footer className="mt-8 flex flex-wrap items-center justify-between gap-4 border-t border-border pt-5 text-[12px] text-muted-foreground">
          ...
          <Link href="/noticias" className="text-primary hover:underline text-[11px] uppercase tracking-widest">← Volver al listado</Link>
        </footer>

        {relacionadas.length > 0 && (
          <section className="mt-10">
            <h2 className="mb-4 text-[10px] uppercase tracking-[0.24em] text-primary">Noticias relacionadas</h2>
            <div className="grid gap-px bg-border rounded-xl overflow-hidden md:grid-cols-3">
              {relacionadas.map((r) => (
                <Link key={r.id} href={`/noticias/${r.slug}`} className="bg-background p-4 transition-colors hover:bg-card">
